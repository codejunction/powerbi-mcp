...
def test_rules_fire():
    ...
    check(
        "DL001 FILTER whole table",
        "DL001"
        in rules('CALCULATE(SUM(Sales[Amt]), FILTER(Sales, Sales[Region]="X"))'),
    )
    check(
        "DL002 nested CALCULATE",
        "DL002" in rules("CALCULATE(CALCULATE([Sales]), Sales[Y]=1)"),
    )
    ...
    check(
        "DL007 SUMMARIZE aggregation",
        "DL007" in rules('SUMMARIZE(Sales, Sales[Region], "Tot", SUM(Sales[Amt]))'),
    )
...
def test_clean_is_silent():
    ...
    check(
        "boolean filter has no DL001",
        "DL001" not in rules('CALCULATE(SUM(Sales[Amt]), Sales[Region]="X")'),
    )
    check(
        "FILTER(VALUES(..)) has no DL001",
        "DL001" not in rules("CALCULATE([X], FILTER(VALUES(Sales[Region]), [X] > 0))"),
    )
    check(
        "real function not DL008",
        "DL008" not in rules("SUMX(Sales, Sales[Qty] * Sales[Price])"),
    )
    check(
        "SUMMARIZECOLUMNS not DL007",
        "DL007" not in rules('SUMMARIZECOLUMNS(Sales[Region], "Tot", SUM(Sales[Amt]))'),
    )
    check(
        "dotted function recognized",
        "DL008" not in rules("PERCENTILE.INC(Sales[Amt], 0.9)"),
    )
    check(
        "VAR name not unknown-func",
        "DL008" not in rules("VAR Threshold = 10 RETURN IF([X] > Threshold, 1, 0)"),
    )
...
def test_tokenizer_robustness():
    ...
    check(
        "slash in line comment ignored",
        "DL003" not in rules("SUM(Sales[Amt]) // ratio a/b here"),
    )
    check(
        "slash in block comment ignored",
        "DL003" not in rules("SUM(Sales[Amt]) /* a/b */ "),
    )
...
def test_rewrites_and_summary():
    ...
    hints = {
        h.rule_id
        for h in dax_lint.suggest_rewrites(
            "M", 'CALCULATE([X], FILTER(Sales, Sales[R]="A")) + [Y]/[Z]'
        )
    }
    ...
    check(
        ...
        res.summary.total >= 1 and res.summary.measures_scanned == 2,
    )
