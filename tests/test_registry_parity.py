...
def main():
    ...
    check(
        "every handler exists",
        all(
            callable(getattr(s, f"_handle_{n}", None)) or n in dispatch
            for n in dispatch
        ),
    )
    unknown_writes = s._write_tools - dispatch
    check(
        "read-only write set names real tools",
        not unknown_writes,
        str(sorted(unknown_writes)),
    )
