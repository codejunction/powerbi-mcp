    ...
    "tables": [
        {
            ...
            "columns": [
                ...
                },
                {
                    ...
                    "is_hidden": False,
                    "is_calculated": True,
                    "expression": 'IF(Sales[Amount]>100,"Big","Small")',
                    "description": "size bucket",
                },
            ],
            "measures": [
                {
...
def has_rule(findings, rule_id, obj_contains=None):
    return any(
        f.rule_id == rule_id
        and (obj_contains is None or obj_contains in (f.object or ""))
        for f in findings
    )
...
def test_bpa():
    ...
    check(
        "DIVIDE NOT flagged for Safe Ratio",
        not has_rule(f, "DAX_USE_DIVIDE", "Safe Ratio"),
    )
    check("trailing space flagged", has_rule(f, "NAMING_TRAILING_SPACE", "Safe Ratio "))
    check(
        "no-format measure flagged", has_rule(f, "FORMAT_MEASURE_NO_FORMAT", "Margin %")
    )
    check(
        "no-description measure flagged",
        has_rule(f, "MAINT_MEASURE_NO_DESC", "Margin %"),
    )
    check(
        "no-description column flagged",
        has_rule(f, "MAINT_COLUMN_NO_DESC", "Sales[Amount]"),
    )
    check("rel type mismatch flagged", has_rule(f, "ERR_REL_TYPE_MISMATCH"))
    check(
        "summary counts present",
        res.summary.total == len(f) and res.summary.by_severity.get("error", 0) >= 1,
    )
    ...
    check(
        ...
        all(x.category == "DAX" for x in only_dax.findings) and only_dax.findings,
    )
    warns = run_bpa(MODEL, min_severity="warning")
    check(
        "severity filter works",
        all(x.severity in ("warning", "error") for x in warns.findings),
    )
...
def test_ai_readiness():
    ...
    check(
        "recommends measure descriptions",
        any("description" in rec.lower() for rec in r.recommendations),
    )
...
def test_diff_models():
    ...
    check(
        "identical -> no changes", same.has_changes is False and same.total_changes == 0
    )
    ...
    after["tables"][0]["measures"].append(
        {"name": "New Measure", "expression": "1", "is_hidden": False}
    )
    after["tables"][0]["columns"] = [
        c for c in after["tables"][0]["columns"] if c["name"] != "Qty"
    ]
