...
def test_classifier():
    ...
    check(
        "credentials",
        classify_refresh_error("AccessUnauthorized: credentials failed").id
        == "credentials_expired",
    )
    check(
        "eviction 0xC11C0020",
        classify_refresh_error("error 0xC11C0020 model evicted").id == "model_eviction",
    )
    check(
        "capacity throttle",
        classify_refresh_error(
            "exceeded the capacity limit for semantic model refreshes"
        ).id
        == "capacity_throttle",
    )
    check(
        "gateway",
        classify_refresh_error("The gateway is offline").id == "gateway_unreachable",
    )
...
def test_resources():
    ...
    errs = json.loads(
        asyncio.run(srv._read_resource("powerbi://reference/refresh-errors"))
    )
    check("refresh-errors has rules", "rules" in errs and len(errs["rules"]) > 3)
    check(
        "disable threshold present",
        errs.get("consecutive_failure_disable_threshold") == 4,
    )
...
def test_read_only_mode():
    ...
    check(
        "security_status is NOT a write tool", "security_status" not in srv._write_tools
    )
    ...
    def call(name, args=None):
        req = CallToolRequest(
            method="tools/call", params={"name": name, "arguments": args or {}}
        )
