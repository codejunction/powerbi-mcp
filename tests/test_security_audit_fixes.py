...
def test_redact_secrets():
    ...
    check(
        ...
        server.redact_secrets("hello world", [""]) == "hello world",
    )
...
def test_audit_chain_tamper_and_scrub():
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        al.log_query(
            query="EVALUATE Sales", result_count=1, duration_ms=5, success=True
        )
        al.log_query(
            query="EVALUATE Bad",
            result_count=0,
        ...
        lines = [
            json.loads(l) for l in open(al.log_file, encoding="utf-8") if l.strip()
        ]
        err = lines[-1]["result"]["error"]
        check(
            "stored error scrubbed",
            err and "topsecret9" not in err and "Password=***" in err,
            str(err),
        )
...
def test_hmac_keyed_chain():
    ...
    try:
        with tempfile.TemporaryDirectory() as tmp:
            al = AuditLogger(log_dir=tmp)
            al.log_query(
                query="EVALUATE X", result_count=1, duration_ms=1, success=True
            )
            line = [
                json.loads(l) for l in open(al.log_file, encoding="utf-8") if l.strip()
            ][0]
            ...
            plain = hashlib.sha256(
                json.dumps(ev, sort_keys=True, default=str).encode()
            ).hexdigest()
...
def test_ref_regex_redos_safe():
    ...
    pathological = (
        "A" * 20000 + " "
    )  # long run, no closing bracket -> used to backtrack
    t0 = time.time()
    ...
    check(
        "quoted + bare tables extracted",
        "Sales Data" in tables and "Sales" in tables,
        str(tables),
    )
...
def test_pii_summary_no_raw():
    ...
    raw_present = any(
        "original" in d or "john.doe@example.com" in json.dumps(d)
        for d in summary.get("detections", [])
    )
...
def test_status_pill_no_numeric_band():
    ...
    dax = svg_measures.generate(
        "status_pill",
        value_measure="M",
        thresholds=[{"max": None, "color": "#3FA34D", "label": "OK"}],
    )["dax"]
