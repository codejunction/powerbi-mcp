...
def test_resources():
    ...
    check(
        "schema has Sales table",
        any(t["name"] == "Sales" for t in schema.get("tables", [])),
        str(schema)[:80],
    )
    measures = json.loads(run(srv._read_resource("powerbi://desktop/measures")))
    check(
        "measures listed",
        any(m["name"] == "Total Sales" for m in measures),
        str(measures)[:80],
    )
...
def test_prompts():
    ...
    check(
        "prompts registered",
        len(srv._prompts) >= 5 and "plan_safe_rename" in srv._prompts,
        str(list(srv._prompts)),
    )
    rendered = srv._prompts["plan_safe_rename"]["render"](
        {"old_name": "OldT", "new_name": "NewT"}
    )
    check("rename prompt includes names", "OldT" in rendered and "NewT" in rendered)
    check(
        "rename prompt steers to PBIP",
        "pbip_rename" in rendered.lower() and "deprecated" in rendered.lower(),
    )
    audit = srv._prompts["audit_model"]["render"]({})
    check(
        "audit prompt orchestrates tools",
        "run_bpa" in audit and "audit_ai_readiness" in audit,
    )
...
def test_completion():
    ...
    srv = make_server()
    comp = run(
        srv._complete_argument(CompletionArgument(name="measure_name", value="tot"))
    )
    check("completes measure name", "Total Sales" in comp.values, str(comp.values))
    comp2 = run(
        srv._complete_argument(CompletionArgument(name="table_name", value="sal"))
    )
    ...
    comp3 = run(
        srv2._complete_argument(CompletionArgument(name="measure_name", value="x"))
    )
...
def test_structured_output():
    ...
    check(
        "run_bpa returns (text, dict)",
        isinstance(text, str) and isinstance(structured, dict),
    )
    check(
        "structured has summary+findings",
        "summary" in structured and "findings" in structured,
    )
