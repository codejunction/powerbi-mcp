...
def test_tmdl_quoting():
    ...
    for name, expected, description in tests:
        ...
        if result == expected:
            ...
            log_fail(
                f"needs_tmdl_quoting('{name}')", f"Expected {expected}, got {result}"
            )
    ...
    for name, expected in quote_tests:
        ...
        if result == expected:
            ...
            log_fail(
                f"quote_tmdl_name('{name}')", f"Expected '{expected}', got '{result}'"
            )
...
def test_rename_with_quoting():
    ...
    try:
        ...
        relationships_folder = os.path.join(
            semantic_folder, "definition", "relationships"
        )
...
"""
        with open(
            os.path.join(relationships_folder, "rel_salesforce_to_projects.tmdl"), "w"
        ) as f:
        ...
        if fix_result["count"] > 0:
            log_pass(
                "Fix DAX",
                f"Fixed {fix_result['count']} references in {len(fix_result['files_modified'])} file(s)",
            )
        else:
            print(
                f"    Info: No DAX quoting fixes needed (count={fix_result['count']})"
            )

        # Verify after fix
        errors_after = connector.validate_tmdl_syntax()
        dax_errors_after = [
            e for e in errors_after if e.error_type == "UNQUOTED_TABLE_IN_DAX"
        ]
        ...
        if result.success:
            log_pass(
                "Table rename",
                f"Renamed with {result.references_updated} references updated",
            )
        else:
        ...
        if "table 'Sales Force Data'" in renamed_content:
            ...
            log_fail(
                "Table declaration", f"Not properly quoted. Content:\n{renamed_content}"
            )
        ...
        with open(
            os.path.join(relationships_folder, "rel_salesforce_to_projects.tmdl"), "r"
        ) as f:
            relationship_content = f.read()

        if (
            "'Sales Force Data to dim_projects'" in relationship_content
            or "'Sales Force Data' to 'dim_projects'" in relationship_content
            or "to dim_projects" in relationship_content
        ):
            log_pass(
                "Relationship name update", "Contains updated table name reference"
            )
        else:
            log_fail(
                "Relationship name update",
                f"Relationship name not updated. Content:\n{relationship_content}",
            )
        ...
        if "fromTable: 'Sales Force Data'" in relationship_content:
            ...
            log_fail(
                "Relationship fromTable update",
                f"Not properly quoted or updated. Content:\n{relationship_content}",
            )
...
"""
        ...
        with open(os.path.join(relationships_folder, "rel1.tmdl"), "w") as f:
            f.write(
                """relationship 'Salesforce_Data to dim_projects'
  cardinality: manyToOne
  fromTable: Salesforce_Data
  fromColumn: ProjectId
  toTable: dim_projects
...
"""
        ...
        with open(os.path.join(relationships_folder, "rel2.tmdl"), "w") as f:
            f.write(
                """relationship rel_SalesforceData_Projects
  cardinality: manyToOne
  fromTable: 'Salesforce_Data'
  fromColumn: ProjectId
  toTable: 'dim_projects'
...
"""
        ...
        result = connector.rename_table_in_files(
            "Salesforce_Data", "Salesforce Customer Data"
        )

        if result.success:
            log_pass(
                "Rename with relationships",
                f"Renamed with {result.references_updated} references",
            )
        else:
        ...
        if "'Salesforce Customer Data to dim_projects'" in rel1_content:
            ...
            log_fail(
                "Relationship 1 name",
                f"Not updated correctly. Content:\n{rel1_content}",
            )
        ...
        if "fromTable: 'Salesforce Customer Data'" in rel1_content:
            ...
            log_fail(
                "Relationship 1 fromTable",
                f"Not properly quoted. Content:\n{rel1_content}",
            )
        ...
        if "fromTable: 'Salesforce Customer Data'" in rel2_content:
            ...
            log_fail(
                "Relationship 2 fromTable", f"Not updated. Content:\n{rel2_content}"
            )
...
"""
        ...
        with open(
            os.path.join(relationships_folder, "rel_sales_projects.tmdl"), "w"
        ) as f:
            f.write(
                """relationship 'Sales to Projects'
  cardinality: manyToOne
  fromTable: Sales
...
"""
        ...
        with open(
            os.path.join(relationships_folder, "rel_sales_projects.tmdl"), "r"
        ) as f:
            rel_content = f.read()

        if (
            "fromColumn: ProjectId" in rel_content
            or "fromColumn: 'ProjectId'" in rel_content
        ):
            log_pass("Relationship fromColumn update", "Column renamed in relationship")
        else:
            log_fail(
                "Relationship fromColumn update",
                f"Column not updated. Content:\n{rel_content}",
            )
        ...
        if "fromTable: Sales" in rel_content:
            ...
            log_fail(
                "Relationship fromTable unchanged",
                f"Table reference was modified. Content:\n{rel_content}",
            )
...
def main():
    ...
    results.append(
        ("Column Rename in Relationships", test_column_rename_in_relationships())
    )
