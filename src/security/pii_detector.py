...
class PIIDetector:
    ...
    def mask_value(
        self, value: str, pii_type: PIIType, strategy: Optional[MaskingStrategy] = None
    ) -> str:
        """
    ...
    def process_value(
        ...
        if column_name:
            ...
            strategy = self.column_overrides.get(
                column_name.lower().strip("[]"), self.default_strategy
            )
        ...
        if pii_found:
            ...
            for pii_type, matched, start, end in pii_found:
                ...
                processed_value = (
                    processed_value[:start] + masked + processed_value[end:]
                )
    ...
    def process_results(
        ...
        summary = {
            ...
            "detections": (
                all_detections if len(all_detections) <= 10 else all_detections[:10]
            ),
        }

        if all_detections:
            logger.info(
                f"PII Detection: Found {len(all_detections)} PII instances across {summary['rows_affected']} columns"
            )
...
def mask_pii(
    results: List[Dict[str, Any]], strategy: MaskingStrategy = MaskingStrategy.PARTIAL
) -> List[Dict[str, Any]]:
    """
