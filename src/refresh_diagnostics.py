...
CONSECUTIVE_FAILURE_DISABLE_THRESHOLD = (
    4  # Power BI auto-disables a schedule after 4 consecutive failures.
)
...
def classify_refresh_error(error_text: Optional[str]) -> RefreshDiagnosis:
    ...
    if not text.strip():
        return RefreshDiagnosis(
            id="none", cause="No error text provided.", remediation="", matched=False
        )
